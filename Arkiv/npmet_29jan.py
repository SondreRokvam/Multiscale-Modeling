...
def get_stiffness(dL,id,stepName,jobName,instanceName,lagrestiffma,pathy):
    mod = mdb.models[modelName]
    a = mod.rootAssembly
    ...
    for i in range(0,6):#   arg:   +   ,len(id)+1
        ...
        #print min(stresses), max(stresses)
        odb.close()
        for k in range(0,6):
...
"""PARAMETERE"""
seed(1) #randomfunksjonens nokkel
Vf = 0.6
nf = 4
r = 1.0
rtol = 0.025 * r
...
Toying = ['Exx'+str(nf), 'Eyy'+str(nf), 'Ezz'+str(nf), 'Exy'+str(nf), 'Exz'+str(nf), 'Eyz'+str(nf)]  # Enhetstoyingene fra 0 til 5. Altsaa alle 6
...
get_stiffness(dL, id, stepName, Toying, instanceName, lagrestiffma,workpath)
